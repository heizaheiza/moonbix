...
class Tapper:
        ...
        try:
            ...
            while True:
                try:
                    ...
                    logger.warning(f"<light-yellow>{self.session_name}</light-yellow> | 限流等待 {fl}")
                    logger.info(f"<light-yellow>{self.session_name}</light-yellow> | 休眠 {fls}秒")
            ...
            traceback.print_exc()
            logger.error(f"<light-yellow>{self.session_name}</light-yellow> | 授权期间发生错误: "
                         f"{error}")
    ...
    def setup_session(self, session: cloudscraper.CloudScraper):
        ...
        if data_['code'] == '000000':
            logger.success(f"{self.session_name} | <green>成功获取访问令牌</green>")
            self.access_token = data_['data']['accessToken']
        else:
            logger.warning(f"{self.session_name} | <red>获取访问令牌失败: {data_}</red>")
    ...
    def get_game_data(self):
        ...
        try:
            ...
            traceback.print_exc()
            logger.error(f"{self.session_name} | <red>获取游戏数据时发生未知错误: {str(error)}</red>")
            return False
    ...
    async def get_user_info(self, session: cloudscraper.CloudScraper):
        ...
        if data_['code'] == '000000':
            ...
            if data__['participated'] is False:
                ...
                logger.info(
                    f"{self.session_name} | 总分数: <yellow>{data__['metaInfo']['totalGrade']}</yellow> | <white>通过风险检测: <red>{data__['riskPassed']}</red> | 资格认证: <red>{data__['qualified']}</red></white>")
    ...
    def get_user_info1(self, session: cloudscraper.CloudScraper):
        ...
        if data_['code'] == '000000':
            ...
            logger.warning(f"{self.session_name} | <red>获取票据数据失败: {data_}</red>")
    ...
    def complete_game(self, session: cloudscraper.CloudScraper):
        ...
        if data_['success']:
            logger.success(
                f"{self.session_name} | <green>成功获取: <yellow>{self.game['log']}</yellow> 来自游戏 !</green>")
        else:
            logger.warning(
    ...
    async def play_game(self, session: cloudscraper.CloudScraper):
        ...
        if ticket_data['metaInfo']['totalAttempts'] == ticket_data['metaInfo']['consumedAttempts']:
            logger.warning(f"{self.session_name} | 没有剩余次数...")
            return
        attempt_left = ticket_data['metaInfo']['totalAttempts'] - ticket_data['metaInfo']['consumedAttempts']
        logger.info(f"{self.session_name} | 开始游戏...")
        while attempt_left > 0:
            # await asyncio.sleep(1000)
            logger.info(f"{self.session_name} | 剩余游戏次数: <cyan>{attempt_left}</cyan>")
            payload = {
                "resourceId": 2056
            }
            ...
            if response.status_code == 200:
                    ...
                    if cap_res.status_code == 200:
                        ...
                        if solve['ok']:
                            ...
                            if solver.status_code == 200:
                                ...
                                if captcha_token == "":
                                    logger.warning(
                                        f"{self.session_name} | <yellow>验证码处理失败. 下一轮重试...</yellow>")
                                    sleep_ = uniform(10, 15)
                                    logger.info(f"{self.session_name} | 休眠 {sleep_}秒...")
                                    await asyncio.sleep(sleep_)
                                    continue
                                logger.success(f"{self.session_name} | <green>验证码处理成功 | 花费时间: <cyan>{solve['solveTime']}秒</cyan></green>")
                                headers['Fvideo-Token'] = self.generate_Fvideo_token(196)
                ...
                if data_['success']:
                    logger.success(
                        f"{self.session_name} | <green>游戏 <cyan>{data_['data']['gameTag']}</cyan> 启动成功</green>")
                    ...
                    if check:
                        logger.info(
                            f"{self.session_name} | 需要 <white>{sleep_}秒</white> 完成游戏...")
                        await asyncio.sleep(sleep_)

                        self.complete_game(session)
            else:
                print(response.text)
                logger.warning(f"游戏启动失败: {response.status_code}")
    ...
    while True:
        for tg_client in tg_clients:
            try:
                ...
                logger.error(f"{tg_client.name} | 无效会话")
