...
class Tapper:
    ...
    async def check_proxy(self, http_client: aiohttp.ClientSession, proxy: Proxy):
        try:
            response = await http_client.get(url='https://httpbin.org/ip', timeout=aiohttp.ClientTimeout(10), ssl=True)
            ip = (await response.json()).get('origin')
            logger.info(f"{self.session_name} | 代理IP: {ip}")
            return True
        except Exception as error:
            logger.error(f"{self.session_name} | 代理: {proxy} | 错误: {error}")
            return False
    ...
    def setup_session(self, session: cloudscraper.CloudScraper):
        ...
        if data_['code'] == '000000':
            logger.success(f"{self.session_name} | <green>成功获取令牌</green>")
            self.access_token = data_['data']['accessToken']
        else:
            logger.warning(f"{self.session_name} | <red>获取令牌失败: {data_}</red>")
    ...
    def get_game_data(self):
        ...
        try:
            ...
            picked_bonus = False
            picked = 0
            logger.info(f"{self.session_name} | 正在进行游戏!")
            game_data_payload = []
            score = 0
            # print(obj_type)
            ...
            if len(game_data_payload) > 0:
                ...
                logger.warning(f"{self.session_name} | <yellow>游戏失败, 原因: 超时</yellow>")
                return False
        except Exception as error:
            traceback.print_exc()
            logger.error(f"{self.session_name} | <red>获取游戏数据时出现未知错误: {str(error)}</red>")
            return False
    ...
    def setup_account(self, session: cloudscraper.CloudScraper):
        ...
        if json_d['success']:
            ...
            json_d = res.json()
            if json_d['success']:
                logger.success(f"{self.session_name} | <green>成功设置账户 !</green>")
                login_task = {
                    "resourceId": 2057
                }
                complete = self.complete_task(session, login_task)
                if complete == "done":
                    logger.success(f"{self.session_name} | <green>首次签到成功 !</green>")

        else:
            logger.warning(
                f"{self.session_name} | <yellow>尝试初始化账户时发生未知错误: {json_d}</yellow>")
    ...
    async def get_user_info(self, session: cloudscraper.CloudScraper):
        ...
        if data_['code'] == '000000':
            ...
            if data__['participated'] is False:
                logger.info(f"{self.session_name} | 尝试设置账户...")
                self.setup_account(session)
                await asyncio.sleep(uniform(3, 5))
                await self.get_user_info(session)
            else:
                logger.info(f"{self.session_name} | <cyan>已登录</cyan>")
                logger.info(
                    f"{self.session_name} | 总分: <yellow>{data__['metaInfo']['totalGrade']}</yellow> | <white>通过风险评估: <red>{data__['riskPassed']}</red> | 资格认证: <red>{data__['qualified']}</red></white>")

        else:
            logger.warning(f"{self.session_name} | <red>获取用户数据失败: {data_}</red>")
    ...
    def get_user_info1(self, session: cloudscraper.CloudScraper):
        ...
        if data_['code'] == '000000':
            ...
            logger.warning(f"{self.session_name} | <red>获取游戏次数失败: {data_}</red>")
    ...
    def get_task_list(self, session: cloudscraper.CloudScraper):
        ...
        if data_['code'] == '000000':
            ...
            logger.warning(f"{self.session_name} | <red>获取任务列表失败: {data_}</red>")
            return None
    ...
    def complete_game(self, session: cloudscraper.CloudScraper):
        ...
        if data_['success']:
            logger.success(
                f"{self.session_name} | <green>本次游戏分数: <yellow>{self.game['log']}</yellow>  !</green>")
        else:
            logger.warning(
                f"{self.session_name} | <yellow>完成游戏失败 | {self.game['log']}: {data_}</yellow>")
    ...
    async def play_game(self, session: cloudscraper.CloudScraper):
        ...
        if ticket_data['metaInfo']['totalAttempts'] == ticket_data['metaInfo']['consumedAttempts']:
            logger.warning(f"{self.session_name} | 没有剩余游戏次数...")
            return
        attempt_left = ticket_data['metaInfo']['totalAttempts'] - ticket_data['metaInfo']['consumedAttempts']
        logger.info(f"{self.session_name} | 正在开始游戏...")
        while attempt_left > 0:
            # await asyncio.sleep(1000)
            logger.info(f"{self.session_name} | 剩余次数: <cyan>{attempt_left}</cyan>")
            payload = {
                "resourceId": 2056
            }
            ...
            if response.status_code == 200:
                ...
                if 'sessionId' in data_['data']:
                    ...
                    cap_res = session.post("https://api.commonservice.io/gateway-api/v1/public/antibot/getCaptcha",
                                           verify="C:/Users/Administrator/Desktop/moonbix-bot/.venv/Lib/site-packages/certifi/cacert.pem",
                                           timeout=300,
                    ...
                    if cap_res.status_code == 200:
                        ...
                        ek = captcha_data_['ek']
                        logger.info(f"{self.session_name} | 尝试处理验证码 ({tag})...")
                        ...
                        if solve['ok']:
                            ...
                            if solver.status_code == 200:
                                ...
                                if captcha_token == "":
                                    logger.warning(
                                        f"{self.session_name} | <yellow>处理验证码失败. 下一轮重试...</yellow>")
                                    sleep_ = uniform(10, 15)
                                    logger.info(f"{self.session_name} | 休眠 {sleep_}秒...")
                                    await asyncio.sleep(sleep_)
                                    continue
                                logger.success(f"{self.session_name} | <green>成功处理验证码 | 花费时间: <cyan>{solve['solveTime']}秒</cyan></green>")
                                headers['Fvideo-Token'] = self.generate_Fvideo_token(196)
                ...
                if data_['success']:
                    logger.success(
                        f"{self.session_name} | <green>游戏 <cyan>{data_['data']['gameTag']}</cyan> 进入成功</green>")
                    ...
                    if check:
                        logger.info(
                            f"{self.session_name} | 等待 <white>{sleep_}秒</white> 来完成游戏...")
                        await asyncio.sleep(sleep_)

                        self.complete_game(session)
            else:
                print(response.text)
                logger.warning(f"游戏启动该失败: {response.status_code}")
    ...
    async def run(self, proxy: str | None) -> None:
        ...
        if proxy:
            ...
            if proxy_check:
                ...
                logger.info(f"{self.session_name} | 绑定代理IP: {proxy}")
        ...
        while True:
            try:
                ...
                if self.access_token:
                    ...
                    if settings.AUTO_TASK:
                        ...
                        for task in task_list:
                            ...
                            if check == "done":
                                logger.success(
                                    f"{self.session_name} | <green>成功完成任务 <cyan>{task['type']}</cyan> | 获得: <yellow>{task['rewardList'][0]['amount']}</yellow></green>")
                            else:
                                logger.warning(
                                    f"{self.session_name} | <light-yellow> 完成任务失败: {task['type']}, 失败信息: {check}</light-yellow>")
                            await asyncio.sleep(uniform(3, 5))

                if settings.AUTO_PLAY_GAME:
                    await self.play_game(session)

                logger.info(f"<light-blue>==已完成 {self.session_name}==</light-blue>")
                session.close()
                ...
                traceback.print_exc()
                logger.error(f"{self.session_name} | 未知错误: {error}")
    ...
    while True:
        ...
        for query in queryids:
            ...
            try:
                ...
                logger.error(f"{query} | 无效账户")

            sleep_ = randint(settings.DELAY_EACH_ACCOUNT[0], settings.DELAY_EACH_ACCOUNT[1])
            logger.info(f"休眠 {sleep_}秒...")
            await asyncio.sleep(sleep_)

        sleep_ = randint(1500, 2500)
        logger.info(f"<red>休眠 {sleep_}秒...</red>")
        await asyncio.sleep(sleep_)
